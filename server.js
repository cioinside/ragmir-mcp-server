...
function handleRequest(req) {
  ...
  if (method === 'initialize') {
    sendResponse(id, {
      ...
      '',
      '=== MCP TOOLS (use these for text files and queries only) ===',
      'ragmir_create_project(name) — create project',
      'ragmir_write_files_batch(project, files) — upload TEXT files (< 100KB each)',
      'ragmir_search(project, query) — search the knowledge base',
      'ragmir_ingest(project) — rebuild index after HTTP file upload',
      '',
      '=== BINARY FILE UPLOAD (REQUIRED for .docx .pdf .xlsx .pptx .png .jpg .zip etc.) ===',
      'MCP tools CANNOT handle files over ~1KB. Binary files are megabytes.',
      'You MUST use HTTP upload via shell command. NEVER use contentBase64.',
      '',
      'Windows (use curl.exe, included in Windows 10+):',
      '  curl.exe -X POST http://192.168.1.226:8002/upload -F "project=PROJECT" -F "path=FILE.docx" -F "file=@C:\\full\\path\\FILE.docx"',
      '',
      'Linux/Mac:',
      '  curl -X POST http://192.168.1.226:8002/upload -F "project=PROJECT" -F "path=FILE.docx" -F "file=@/full/path/FILE.docx"',
      '',
      'Python (if curl is unavailable):',
      '  import requests; requests.post("http://192.168.1.226:8002/upload", files={"file": open("FILE.docx","rb")}, data={"project":"PROJECT","path":"FILE.docx"})',
      '',
      'After upload, call ragmir_ingest(project="PROJECT") via MCP.',
      'The upload endpoint auto-ingests by default, so ingest is optional.',
    ].join('\n'),
    });
    return;
  }
