...
function handleRequest(req) {
  ...
  if (method === 'initialize') {
    sendResponse(id, {
      ...
      '- ragmir_write_files_batch(project, files) → upload files and auto-index',
      '- ragmir_search(project, query) → search the knowledge base',
      '',
      'HOW TO UPLOAD TEXT FILES (.py, .md, .js, .ts, .json, .yaml, .html, .css, etc.):',
      'Pass the file content directly as the "content" field.',
      'Example:',
      '  ragmir_write_files_batch(project="my-project", files=[{path: "src/main.py", content: "print(\\"hello\\")"}])',
      '',
      'HOW TO UPLOAD BINARY FILES (.docx, .pdf, .xlsx, .pptx, .png, .jpg, etc.):',
      'Binary files cannot be passed as text. You MUST base64-encode them first.',
      'Step 1: Read the file as base64 using a shell command:',
      '  Linux/Mac:  base64 -w0 /path/to/file.docx',
      '  Windows PowerShell:  [Convert]::ToBase64String([IO.File]::ReadAllBytes("C:\\path\\file.docx"))',
      'Step 2: Pass the base64 string as "contentBase64" field:',
      '  ragmir_write_files_batch(project="my-project", files=[{path: "docs/report.docx", contentBase64: "UEsDBBQ..."}])',
      '',
