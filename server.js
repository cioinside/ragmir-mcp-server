...
function handleRequest(req) {
  ...
  if (method === 'initialize') {
    sendResponse(id, {
      ...
      '- ragmir_write_files_batch(project, files) → upload text files',
      '- ragmir_search(project, query) → search the knowledge base',
      '',
      'HOW TO UPLOAD TEXT FILES (.py, .md, .js, .ts, .json, .yaml, .html, .css, etc.):',
      'Use ragmir_write_files_batch directly:',
      '  ragmir_write_files_batch(project="my-project", files=[{path: "src/main.py", content: "print(\\"hello\\")"}])',
      '',
      'HOW TO UPLOAD BINARY FILES (.docx, .pdf, .xlsx, .pptx, images, etc.):',
      'DO NOT pass binary files through MCP tools — they are too large.',
      'Use the HTTP upload endpoint instead. Run this shell command:',
      '',
      '  Linux/Mac:',
      '    curl -X POST http://SERVER_IP:8002/upload -F "project=PROJECT_NAME" -F "path=PATH/IN/PROJECT" -F "file=@/local/path/to/file.docx"',
      '',
      '  Windows PowerShell:',
      '    Invoke-WebRequest -Uri "http://SERVER_IP:8002/upload" -Method POST -Form @{project="PROJECT_NAME"; path="PATH/IN/PROJECT"; file=Get-Content "C:\\path\\file.docx" -Encoding Byte}',
      '',
      'After uploading binary files, call ragmir_ingest(project="PROJECT_NAME") to index them.',
      '',
      'NEVER try to read binary files as text. NEVER extract text from documents yourself.',
      'Ragmir handles document parsing natively during ingestion.',
    ].join('\n'),
    });
    return;
  }
